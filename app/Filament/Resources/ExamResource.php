...
use App\Models\Exam;
use Filament\Forms;
use Filament\Tables\Columns\View;
...
use Filament\Tables\Columns\Layout\Split; // ini penting
...
class ExamResource extends Resource
{
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                Tables\Columns\TextColumn::make('exam_name')->label('Exam Name'),
                Tables\Columns\TextColumn::make('class.class_name')->label('Class'),
                TextColumn::make('subject.name')->label('Subject')->searchable(),
                Tables\Columns\TextColumn::make('examType.exam_type_name')->label('Type'),
                Tables\Columns\TextColumn::make('exam_date')->date('d M Y')->label('Date'),
            ])
            // ->expandable() // ✅ aktifkan expandable row
            // ->expandable(fn(Exam $record) => view('filament.tables.rows.exam-marks', ['record' => $record]))
            // ->expandableColumns([
            //     View::make('filament.tables.rows.exam-marks'), // Custom Blade view
            // ])
            // ->columns([
            ...
            ->filters([])
            ->actions([
                Action::make('view_marks')
                    ->label('View Marks')
                    ->icon('heroicon-o-eye')
                    ->modalHeading(fn(Exam $record) => 'Marks for ' . $record->exam_name)
                    ->modalContent(fn(Exam $record) => view('filament.tables.rows.exam-marks', [
                        'record' => $record,
                    ]))
